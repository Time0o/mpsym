                           ...
                           OrbitGraph &orbit_graph)
{
  ...
  util::ContainerMap<std::vector<unsigned>, unsigned> component_indices{{alpha, 0u}};
  ...
  for (unsigned i = 0u, n = 0u; i < component.size(); ++i) {
    ...
    for (auto j = 0u; j < generators.size(); ++j) {
      ...
      auto beta_prime(
        generators[j].image<std::vector>(beta.begin(), beta.end()));

      auto it(component_indices.find(beta_prime));

      if (it == component_indices.end()) {
        component_indices[beta_prime] = component.size();
        component.push_back(beta_prime);

        ++n;

        schreier_tree_data.emplace_back(i, j);
        ...
        orbit_graph_data[j].push_back(it->second);
        DBG(TRACE) << "g_" << i + 1u << "," << j + 1u << " = " << it->second + 1u;
      }
    }
  }
                           ...
                           unsigned scc_)
{
  ...
  DBG(TRACE) << "Finding spanning tree for s.c.c rooted at node " << scc_ + 1u;

  DBG(TRACE) << "In orbit graph:\n" << orbit_graph;
  ...
  std::vector<unsigned> vertex_map(sccs.data.size(), 0u);

  DBG(TRACE) << "Constructing temporary subgraph";

  for (unsigned i = 0u; i < sccs.data.size(); ++i) {
    if (sccs.data[i] == sccs.data[scc_]) {
      scc.push_back(i);

      unsigned v = boost::add_vertex({i}, g);
      vertex_map[i] = v + 1u;

      DBG(TRACE) << "Added vertex " << v << " (" << i + 1u << ')';
    }
  }

  assert(scc[0] == scc_ && "provided representative is first node in s.c.c");

  for (unsigned source : scc) {
  ...
  boost::random_spanning_tree(
    g, re, boost::root_vertex(vertex_map[scc_] - 1u).predecessor_map(&pred[0]));
  ...
  spanning_tree.data =
    std::vector<std::pair<unsigned, unsigned>>(sccs.data.size() - 1u);

  for (auto i = 1u; i < pred.size(); ++i) {
    auto child(boost::vertex(i, g));
    auto parent(boost::vertex(pred[i], g));
...
PartialPerm schreier_trace(PartialPermSet const &generators,
                           ...
                           unsigned to)
{
  PartialPerm res(generators.dom_max());

  while (from != to) {
    unsigned v = std::get<0>(schreier_tree.data[from - 1u]);
    unsigned w = std::get<1>(schreier_tree.data[from - 1u]);
...
std::ostream &operator<<(std::ostream &os, eemp::OrbitGraph const &orbit_graph)
{
  ...
  os << "s.c.c." << std::string(pad - 1u, ' ') << " | "
     << strongly_connected_components(orbit_graph);

  return os;
}

std::vector<std::vector<unsigned>> Sccs::data_expanded() const
{
  ...
  std::vector<std::vector<unsigned>> expanded(last - first + 1);
  for (unsigned x = 0u; x < data.size(); ++x)
    expanded[data[x] - first].push_back(x);
...
{
  os << DUMP_CUSTOM(sccs.data_expanded(), "{}", "{}");
  return os;
}
