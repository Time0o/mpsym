...
using cgtl::Perm;
using cgtl::PermSet;
using cgtl::SchreierStructure;

void normalizing_generator(Perm const &gen, BSGS &bsgs)
{
  ...
  while (!h.id()) {
    ...
    if (i > bsgs.base.size()) {
      for (unsigned j = 1u; j <= bsgs.degree; ++j) {
        if (h[j] != j) {
          bsgs.extend_base(i);
          break;
        }
    ...
    if (m > 1u) {
      ...
      for (unsigned j = 0u; j < i; ++j) { // TODO: avoid complete recomputation
        PermSet s_j(bsgs.schreier_structures[j]->labels());
        s_j.add(h);

        bsgs.update_schreier_structure(j, s_j.vect());
...
bool s_normal_closure(Perm const &w,
...
void solve_bsgs(BSGS &bsgs, PermSet const &generators)
{
  ...
  unsigned iterations =
    static_cast<unsigned>(5.0 / 2.0 * std::log(bsgs.degree) / std::log(3.0));
  ...
  for (auto const &gen : generators) {
    ...
    while (!bsgs.strips_completely(gen)) {
      ...
      for (unsigned i = 0u; i < iterations; ++i) {
        ...
        std::pair<Perm, Perm> conjugates;
        success = s_normal_closure(w, generators, bsgs, conjugates);
        if (success)
          break;
      ...
      if (!success) {
        Dbg(Dbg::DBG) << "==> Failure";
        throw BSGS_solve_error("BSGS not solvable");
      }
    }
  }
