...
TEST_P(DisjointSubgroupProductTest, CanFindDisjointSubgroupProduct)
{
  ...
  for (auto i = 0u; i < permgroups.size(); ++i) {
    ...
    ASSERT_EQ(expected_disjoint_subgroups[i].size(), disjoint_subgroups.size())
      << "Correct number of disjoint subgroups generated.";

    std::vector<bool> found_subgroups(expected_disjoint_subgroups[i].size(), false);

    for (auto const &subgroup : disjoint_subgroups) {
      bool is_expected_subgroup = false;
      for (auto j = 0u; j < expected_disjoint_subgroups[i].size(); ++j) {
        if (found_subgroups[j])
          continue;

        if (perm_group_equal(expected_disjoint_subgroups[i][j], subgroup)) {
          EXPECT_FALSE(found_subgroups[j])
            << "Subgroup not returned more than once.";

          found_subgroups[j] = true;
      ...
      if (!is_expected_subgroup) {
        ...
        std::stringstream ss;
        ss << '[' << generators[0];
        for (auto k = 1u; k < generators.size(); ++k)
          ss << ", " << generators[k];
        ss << ']';
